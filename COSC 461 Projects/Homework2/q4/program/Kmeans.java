...
public class Kmeans
{
    ...
    private double[][] normalizedRecords;
    ...
    {
        normalizedRecords = new double[numberRecords][numberAttributes];
        //minimum and maximum values are given in the header of file7.txt
        double[] minValues = {20, 20, 500};
        double[] maxValues = {100, 100, 900};

        //Normalize records between the range of 0 and 1
        for(int i = 0; i < numberRecords; i++)
        {
            for(int j = 0; j < numberAttributes; j++)
            {
                normalizedRecords[i][j] = (records[i][j] - minValues[j]) / (maxValues[j] - minValues[j]);
            }
        }
    ...
    private void initializeCentroids()
    {
        ...
        for (int i = 0; i < numberClusters; i++)
        {
            ...
            for (int j = 0; j < numberAttributes; j++)
                centroids[i][j] = normalizedRecords[index][j];
        }
    }    
    ...
    private void assignClusters()
    {
        ...
        for (int i = 0; i < numberRecords; i++)
        {
            ...
            double minDistance = distance(normalizedRecords[i], centroids[0]);
            int minIndex = 0;
            ...
            for (int j = 0; j < numberClusters; j++)
            {
                ...
                double distance = distance(normalizedRecords[i], centroids[j]);
    ...
    private void updateCentroids()
    {
         ...
         for (int i = 0; i < numberRecords; i++)
         {
             ...
             clusterSum[cluster] = sum(clusterSum[cluster], normalizedRecords[i]);
    ...
    //Method to calculate the sum squared error
    public double findSSE()
    {
        ...
        for(int i = 0; i < numberRecords; i++)
        {
            //get the cluster that the record is assigned to
            int clusterIndex = clusters[i];

            //Calculate the squared distance between the record and its centroid
            double squaredDistance = distance(normalizedRecords[i], centroids[clusterIndex]);

            //add the squared distance to the SSE
            sse+= squaredDistance;
        }
    ...
    public void display(String outputFile) throws IOException
    {
         ...
         outFile.println("Number of clusters: "+ numberClusters + "\n");

         //for each cluster
         for(int c = 0; c < numberClusters; c++)
         {
            //Print the cluster number
            outFile.println("Cluster "+ (c + 1) + ":");

            //Iterate over all records
            for(int i = 0; i < numberRecords; i++)
            {
                //If the record belonds to the current cluster
                if(clusters[i] == c)
                {
                    //Write attributes of the record
                    for(int j = 0; j < numberAttributes; j++)
                    {
                        outFile.print(records[i][j] + " ");
                    }

                    //Move to the next line after writing all attributes of the record
                    outFile.println();
                }
            }

            //Add extra line for better separation
            outFile.println();
         }
