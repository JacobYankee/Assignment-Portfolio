...
public class SlidingAstar
{
    ...
    private int evaluation;               //evaluation option
    private int heuristicF;               //hueristic option
    private int swapCounter;              //number of swaps
    private int searchCounter;            //number of searches 
    private PrintWriter fileOutput;       //print writer to write to file
    private long startTime;               //start time for solve
    private long endTime;                 //end time for solve
    ...
    public SlidingAstar(int[][] initial, int[][] goal, int size, int evaluation, int heuristicF, int swapCounter, int searchCounter, String filePath) throws FileNotFoundException
    {
        ...
        this.evaluation = evaluation;                   //set evaluation option
        this.heuristicF = heuristicF;                   //set heuristic function
        this.fileOutput = new PrintWriter(filePath);    //set file output
        this.swapCounter = 0;                           //set swap counter equal to zero
        this.searchCounter = 0;                         //set search counter equal to zero
    }
    ...
    public void solve()
    {
        ...
        startTime = System.currentTimeMillis();
         ...
         {
            swapCounter--;      //subtract swapcounter by 1 to account for the initial board print counting as a swap
            //Print run stats
            System.out.println("Time: "+runTime + "\nSearches: "+ searchCounter + "\nSwaps: "+ swapCounter);
            
            fileOutput.println("Time: "+runTime + "\nSearches: "+ searchCounter + "\nSwaps: "+ swapCounter);
         }
      fileOutput.flush();   
    }                                                
    ...
    private Board createChild(Board board, int i, int j, char direction)
    {
        ...
        if(this.heuristicF == 1)                     //heuristic value of child
        child.hvalue = heuristic_M(child);           
        else if(this.heuristicF == 2)
        ...
        {
            case 1:
            child.fvalue = child.hvalue;        //f = h where h is heuristic function
            break;
            case 2:
            child.fvalue = child.gvalue;        //f = g where g is path cost function
            break;
            case 3:
        ...
        this.searchCounter++;       //increase search count
    ...
    private void displayBoard(Board board)
    {
        ...
        System.out.println(); 
        fileOutput.println();
        this.swapCounter++;     //increase swap count  
