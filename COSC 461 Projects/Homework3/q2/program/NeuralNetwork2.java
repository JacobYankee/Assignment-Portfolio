...
public class NeuralNetwork2
{
    ...
    int seed)
    {
        ...
        //return the seed as no other method uses it
        return seed;
    }
    ...
    public void testData(String inputFile, String outputFile) throws IOException
    {
         ...
         for (int i = 0; i < numberRecords; i++)
         {
             ...
             for (int j = 0; j < numberOutputs; j++)
                 outFile.print(numToClass(output[j]) + " ");
             outFile.println();
         }
    ...
    public double validate(String validationFile) throws IOException
    {
         ...
         for (int i = 0; i < numberRecords; i++)
         {
             ...
             for (int j = 0; j < numberOutputs; j++)
                 System.out.print(numToClass(actualOutput[j]) + " ");
             System.out.println();

             //display predicted outputs
             for (int j = 0; j < numberOutputs; j++)
                 System.out.print(numToClass(predictedOutput[j]) + " ");
             System.out.println();
             System.out.println();
         }
    ...
    //Method to convert number to class
    public String numToClass(double classVal)
    {
        //round down to determine class, use conditionals
        //Class ranges are determined by fourths as class values are odd eighths
        if(0.0 < classVal && classVal <= 0.25)
        return "low";
        else if(0.25 < classVal && classVal <=0.50)
        return "medium";
        else if(0.50 < classVal && classVal <=0.75)
        return "high";
        else
        return "undetermined";
    }
