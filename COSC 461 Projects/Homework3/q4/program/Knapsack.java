...
public class Knapsack
{
    ...
    public Knapsack(String inputPath) throws FileNotFoundException 
    {
        //set knapsack data
        readFile(inputPath);
    }
    ...
    public int setParameters(int populationSize, int stringLength,
    int numberIterations, double crossoverRate, double mutationRate, int seed)
    {
        ...
        //Return the seed as setParameters is the only method to use the seed
        return seed;
    }
    ...
    public void solve(String outputPath) throws IOException
    {
         ...
         writeResults(outputPath, solution);
    }
    ...
    //Method writes solution and parameters to output file
    private void writeResults(String outputPath, int[] solution) throws IOException
    {
        PrintWriter outF = new PrintWriter(new FileWriter(outputPath));
        int price = 0;
        int profit = 0;
        for(int i = 0; i < solution.length; i++)
        {
            if(solution[i] == 1)
            {
                outF.println(table[i][0]);
                price += table[i][1];
                profit += (table[i][2] - table[i][1]);
            }
        }

        outF.println("\nTotal buying price: "+price +"\nTotal expected profit: "+profit+"\nPopulation size: "+populationSize+"\nString length: "+stringLength+"\nCrossover rate: "+crossoverRate+"\nMutation rate: "+mutationRate+"\nIterations: "+numberIterations);
        outF.close();
    }
    ...
    private void readFile(String inputPath) throws FileNotFoundException
    {
        Scanner scan = new Scanner(new File(inputPath));
        numberItems = scan.nextInt();
        maximumWeight = scan.nextInt();
        table = new int[numberItems][3];
        //Loop through file, add data to array
